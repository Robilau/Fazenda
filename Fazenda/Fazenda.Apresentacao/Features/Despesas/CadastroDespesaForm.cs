...
namespace Fazenda.Apresentacao.Features.Despesas
{
    public partial class CadastroDespesaForm : Form
    {
        ...
        {
            InitializeComponent();
            numValorUnitario.Increment = 0.01m;
        ...
        public Despesa novaDespesa
        {
            get
            {
                ...
                _despesa.ValorUnitario = Convert.ToDouble(numValorUnitario.Value);
                return _despesa;
            }
            set
            {
                ...
                numValorUnitario.Value = (decimal)_despesa.ValorUnitario;
            }
        }
        ...
        {
            var animais = _animalServico.PegarTodos();
        ...
        {
            var animais = _animalServico.PegarTodos().Where(a => a.Especie.ToUpper().Contains(cmbAnimal.Text.ToUpper()));
            cmbAnimal.Items.Clear();
            foreach (var item in animais)
            {
                cmbAnimal.Items.Add(item);
        ...
        {
            var itens = _itemServico.PegarTodos();
        ...
        {
            var itens = _itemServico.PegarTodos().Where(i => i.Descricao.ToUpper().Contains(cmbItens.Text.ToUpper()));
            cmbItens.Items.Clear();
            foreach (var item in itens)
            {
                cmbItens.Items.Add(item);
        ...
        {
            var fornecedores = _fornecedorServico.PegarTodos();
        ...
        {
            var fornecedores = _fornecedorServico.PegarTodos().Where(f => f.Nome.ToUpper().Contains(cmbFornecedor.Text.ToUpper()));
            cmbFornecedor.Items.Clear();
            foreach (var item in fornecedores)
            {
                cmbFornecedor.Items.Add(item);
        ...
        {
            txtUnidade.Text = ((Item)(cmbItens.SelectedItem)).Unidade;
        }
